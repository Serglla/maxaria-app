...
### Sesión Cowork: botón Clave, catálogo sin precios, tickets en dashboard, cobro del tercerizado y lista al crear cliente (12 agosto 2026 — `admin.js?v=20260812a`)

Cinco pedidos encadenados de Sergio, todos en la misma sesión de Cowork. **El bash mount NO estuvo stale en ningún momento**: `node --check` coincidió con Read y no hubo truncamientos. Cache busting final unificado `admin.js?v=20260812a` (cubre las cinco cosas; el CSS no se tocó). Todo en disco local, **sin `git add/commit/push` ni deploy en Railway**.

**1. El botón 🔑 Clave salió del topbar y vive en la fila del superadmin**
- Decisión de Sergio (AskUserQuestion): **sacarlo del topbar para todos**, no ocultarlo solo para él.
- `admin.html`: se eliminó `<button id="mypass-btn">` del `.topbar-actions`. El modal `#mypass-modal` quedó intacto.
- `admin.js`: `mypassEls` ya no tiene `btn`; el bloque de wiring pasó a `if (mypassEls.modal && mypassEls.form)` y la apertura se extrajo a **`function openMypassModal()`** — declaración (no `const`) **a propósito**, para que quede hoisteada: la llama el handler de `els.adminsTbody`, que está ~7.500 líneas más arriba en el archivo.
- `renderAdmins`: la fila del superadmin tenía una celda vacía (`—`) porque a propósito no se puede resetear desde ahí. Ahora, **solo si `a.id === state.me.id`**, muestra un botón `data-act="mypass"` que abre el modal de cambiar la propia contraseña (pide la actual, no es un reset). Si hubiera otro superadmin, su fila sigue con `—`.
- **Efecto colateral asumido**: los admins limitados (laura, mary) ya no tienen forma de cambiar su propia clave desde el panel — se las resetea el superadmin con el botón Clave de su fila.

**2. Catálogo PDF sin precios (`withPrices`)**
Pedido: "un catálogo sin precios, solo la lista, con o sin imágenes".
- `server.js` `POST /api/admin/catalog/pdf`: `const withPrices = body.withPrices !== false` (default true). **`includePriceChanges` pasó a ser `withPrices && !!body.includePriceChanges`** (sin precios no tiene sentido adjuntar la sección de cambios, que es la única otra parte del PDF que imprime `$`). El nombre del archivo agrega `-sin-precios`. El encabezado dice **"Lista de productos"** en vez de "Precios: <lista>".
- Tarjeta con imagen: el nombre gana altura (`height` 24→38) y más caracteres (46→70) porque no hay que reservar el renglón grande del importe; código y descripción bajan su `y`; el bloque del precio queda dentro de `if (withPrices)`.
- Fila compacta: `priceW` pasa a 0 y el nombre se queda con todo el ancho (55→78 chars).
- **Bug preexistente corregido de paso**: el nombre de la fila compacta se dibujaba con `lineBreak:false` **sin `ellipsis`**, y en pdfkit 0.18 eso igual wrapea → un nombre largo se iba a 2-3 líneas y pisaba el precio y la fila de abajo. Medido: el límite de 55 caracteres daba 247pt en una celda de 132pt. Ahora lleva `{ width: nameW, height: 11, lineBreak: false, ellipsis: true }` → una sola línea cortada con "…". **`ellipsis` NO se activa sin `height`** (probado: sin `height` sigue wrapeando).
- `admin.html`: checkbox `#catalog-with-prices` (checked por default) arriba del selector de lista; el bloque de "incluir cambios de precio" se envolvió en `#catalog-changes-wrap`.
- `admin.js`: al destildar, `syncCatalogClientUI()` oculta `#catalog-price-wrap` y `#catalog-changes-wrap` y cambia el hint del cliente ("Sin precios: el cliente solo define qué categorías se incluyen" — el selector de cliente sigue sirviendo para heredar categorías). `fetchCatalogPdfBlob` manda `withPrices`; `catalogFileName()` refleja el sufijo.
- **Verificación**: se generaron los 4 PDFs de verdad (con/sin precios × con/sin imágenes) extrayendo el bloque del catálogo con `sed -n '9714,9829p' server.js > body.js` y ejecutándolo con `eval` dentro de un harness con pdfkit real + `byCategory` falso. Texto extraído de los content streams: los "sin precios" **no contienen ningún `$`**. Ojo al extraer: pdfkit emite **arrays con kerning** `[<hex> 20 <hex> 0] TJ`, así que el regex tiene que ser `\[(.*?)\]\s*TJ` y concatenar todos los `<hex>` (decodificar en **latin-1**, no utf-16-be).

**3. Dashboard: tarjeta "Pedidos del mes" con ticket promedio**
Sergio pidió la cantidad de pedidos del mes ("los tickets de ventas") con % vs mes anterior. Se le mostró un mockup con 3 variantes (widget) antes de codear; eligió **tarjeta nueva** + **incluir ticket promedio** (le costó entender qué era el ticket promedio: se le explicó con un ejemplo de dos meses — sirve para saber si crecés por cantidad de pedidos o por tamaño de cada uno).
- `server.js` `/api/admin/dashboard`: `salesPrevMonth` ahora hace `COUNT(*) AS cnt` además del SUM, y lo devuelve en el JSON. **`salesPrevMonth` ya venía cortado al mismo día del mes** (`prevMonthCut`), así que las dos comparaciones son parejas sin tocar nada más.
- `admin.html`: tarjeta `dk-indigo` entre "Este mes" y "Cobros hoy", con `#dash-orders-month` + **dos** `.dash-kpi-sub` (`#dash-orders-month-vs` y `#dash-ticket-month`). La fila de Ventas pasó a 6 tarjetas (con `flex-wrap`, en pantallas chicas wrapea sola).
- `admin.js` `loadDashboard`: helper local `pctTxt(curr, prev)` (▲/▼ + %), cantidad de pedidos con "▲ 15% vs mes ant. (19)" y, si el mes anterior fue 0, **"sin datos del mes anterior"** en vez de un porcentaje inventado. Ticket promedio = `total / cnt` con su propia variación.

**4. 🔴 Cobro de un pedido con vendedor tercerizado: precarga el neto y salda al cliente**
Reporte de Sergio (pedido #241, Walter, vendedor Juan Manuel tercerizado, comisión $49.794): el modal **"Registrar cobro"** precargaba **$994.938** (la deuda completa) cuando Juan le cobra al cliente, se queda su comisión y **rinde $945.144**. Era el pendiente que había quedado anotado el 18 jun ("precargar lo que rinde Juan") — el modelo "rinde neto" se había hecho el 20 jul **solo para el modal de Entrega**; el path de Cobro (`POST /api/admin/payments`) seguía con el modelo viejo (cobrás todo + egreso de comisión). Decisión (AskUserQuestion): aplicar **solo a tercerizados** (con vendedor propio se sigue cobrando el total y la comisión sale como egreso).
- **Helper nuevo `syncRindeNetoCredit(orderId)`** en `server.js`, justo después de `syncVendorCommissionEgreso`. Borra siempre el crédito `'Comisión rendida%'` del pedido y, si el vendedor asignado es tercerizado con comisión > 0, lo recrea proporcional a **TODO lo cobrado** del pedido (`cashCollectedForOrder` = entregas + pagos) sobre `netRinde = total − discount_amount − comisión`. Devuelve `null` cuando no aplica (ahí la comisión va como egreso de caja). Idempotente; debe llamarse dentro de la transacción, después de insertar el cobro y **después** de guardar el descuento del pedido.
- **Se unificaron las dos rutas**: `POST /api/orders/:id/deliver` perdió su bloque inline de "Comisión rendida" (que solo miraba el `cobrado` de esa entrega) y ahora llama a `syncRindeNetoCredit(id)` al final de la transacción, junto al bloque de comisión. Así una **entrega parcial + un cobro posterior** no duplican ni subestiman el crédito (verificado: queda 1 solo movimiento, actualizado al total).
- `POST /api/admin/payments` y `DELETE /api/admin/payments/:id`: `if (rinde) { borrar cash_movements source='comision' } else { syncVendorCommissionEgreso(...) }`. Borrar el cobro recalcula y revierte.
- **Frontend** (`admin.js`): `state.payForOrder` ganó `is_tercerizado` (de `order.profitability.vendor.is_tercerizado`) y `discount` (de `profitability.discount`). Helper nuevo **`payPendingCommission()`** — espejo del server: comisión − parte ya acreditada. `openPaymentForOrder` precarga `max(0, due − pendiente)`. `renderPaySplit()` para tercerizado muestra "El vendedor (X) se queda su comisión: $C · a tu caja entra $Y (se acredita igual en la cuenta del cliente, queda saldado)" en vez del split "primero lo tuyo". Se agregó un listener `input` en `[name="amount"]` del form de pago (antes el split no se refrescaba al tipear, y como `renderPaySplit()` corría antes del `.then()` que precarga el monto, **no se veía nunca**).
- **Verificación** (sqlite3 en memoria con los números reales del #241): cobro del neto $945.144 → crédito de comisión $49.794 → **cliente saldado en $0**; cobro parcial de la mitad → acredita $24.897 y queda debiendo $497.469; borrar el pago revierte a la deuda original; vendedor propio → devuelve `null` (sin cambios de comportamiento); entrega + cobro → 1 solo crédito de comisión.
- **Distinción que sigue vigente**: esto es SEPARADO del campo "Descuento / comisión" del modal (ese le baja la deuda AL CLIENTE). La comisión rendida no cambia lo que debe el cliente, solo separa la parte del vendedor de lo que entra a la caja.

**5. Crear usuario: el selector de "Nivel" pasó a ser el de "Lista de precios"**
Sergio: "¿se podrá agregar a las listas o nivel las listas de precios que creamos, como la BC?". El modal de **edición** ya usaba el selector unificado desde el 9 jun; el de **creación** seguía con los 4 niveles pelados, así que había que crear el cliente y después editarlo.
- `admin.html`: el `<select name="level" id="user-create-level">` pasó a `<select name="pricecfg" id="user-create-price">` con los 4 niveles como fallback estático.
- `admin.js`: al abrir el modal se hace `await ensurePriceListsLoaded()` y se rellena con **`unifiedPriceOptsHtml({level:1, price_list_id:null})`** (mismo helper que el modal de edición → optgroups "Nivel base" / "Listas personalizadas"). En el submit, `decodePriceCfg` decodifica `"level:N"` / `"list:ID"`; con una lista, el `level` que se manda es el de su **nivel raíz** vía el mapa nuevo **`PRICE_BASE_LEVEL_NUM`** (`{minorista:1, revendedor:2, mayorista:3, vip:4, publico:1, costo:1}`) — queda como fallback si la lista se desactiva, porque `getEffectivePriceConfig` prioriza `price_list_id`.
- `server.js` `POST /api/admin/users`: acepta `price_list_id` (valida que exista y esté **activa**, y que el nivel sea 1-4), y lo incluye en el INSERT. Antes la columna ni se tocaba en el alta.
- Si no hay listas activas, el selector se ve exactamente como antes.

**Verificación de la sesión**: `node --check` OK en `server.js` y `admin.js` tras cada cambio, con los finales de archivo íntegros (`app.listen` / `bootstrap(); })();`). Tests aislados en `/tmp`: PDFs reales del catálogo, contabilidad del "rinde neto" con sqlite3, decodificación del selector de creación (6 casos), y las queries del dashboard contra copia de la DB local (que **no tiene pedidos de este mes ni listas de precios**, así que esos números hay que mirarlos en producción). Sin navegador en Cowork (better-sqlite3 es binario Windows), el aspecto lo tiene que confirmar Sergio con Ctrl+F5.
